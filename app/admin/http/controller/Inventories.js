...
class Inventories {
  async getAll (req, res) {
    ...
    InventoriesModel.find(
      { name: { $regex: name }, code: { $regex: code } }
    )
      .skip(skip)
      .limit(limit)
  ...
  async create (req, res) {
    ...
    const inventoriesModel = new InventoriesModel({..._.pick(req.body,
      ['name', 'sortOrder', 'code', 'active'])
    })

    inventoriesModel.save()
      .then(result => {
        res.status(200).json(_.pick(result,
          ['_id' ,'name', 'sortOrder', 'code', 'active']))
      })
      .catch(err => {
        res.status(500).json({
          error: err,
  ...
  async update (req, res) {
    ...
    InventoriesModel.findByIdAndUpdate({ _id: id }, _.pick(req.body,
      ['name', 'sortOrder', 'code', 'active']))
      .then(result => {
        res.status(200).json(_.pick(result, ['_id' ,'name', 'sortOrder', 'code', 'active']))
      })
