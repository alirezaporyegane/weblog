...
const { validateUser } = require('../validator/Account')
...
class user {
          ...
          if (err) {
            ...
            user
              ...
              .then((result) => {
                ...
                res
                  .status(200)
                  .json({ ..._.pick(result, ['_id', 'userName', 'phone', 'email']), token: token })
              })
              .catch((err) => {
                res.status(500).json({
                  data: err,
      ...
      bcrypt.compare(req.body.password, user[0].password, (err, result) => {
        if (err) {
          ...
          return res.status(200).json({
            ..._.pick(user[0], ['_id', 'userName', 'phone', 'email']),
  ...
  async sendCode(req, res) {
    User.findOne({ phone: req.user.phone }).then((user) => {
      if (!user)
        return res.status(400).json({
          msg: 'Conflict',
          code: 409
  ...
  async getCode(req, res) {
    ...
    User.findOne({ phone: req.user.phone }).then((result) => {
      if (!code)
        return res.status(400).json({
          msg: 'Bad Request',
          code: 400
